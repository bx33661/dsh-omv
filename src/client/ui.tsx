...
export function Posture({ data }: { data: DashboardPayload }) {
  ...
  return <section className="omv-panel"><div className="omv-panel-head"><div><h3>证据态势</h3><p>按论证成熟度分布，不再求平均分</p></div></div><div className="omv-posture"><div className="omv-posture-score"><strong>{maturity.verified + maturity.supported}</strong><span>条发现已有支撑性证据</span></div><div className="omv-legend"><Legend color="var(--dsw-alias-state-success-primary, #329568)" label="已经验证" value={maturity.verified} /><Legend color="var(--dsw-static-blue-400, #4d6bfe)" label="证据支撑" value={maturity.supported} /><Legend color="var(--dsw-alias-state-warn-primary, #b7791f)" label="正在成形" value={maturity.developing} /><Legend color="var(--dsw-alias-label-tertiary, #8b8f98)" label="尚未映射" value={maturity.unmapped} /><Legend color="var(--dsw-alias-state-error-primary, #d44c4c)" label="存在争议" value={maturity.contested} /></div><div className="omv-alert">{needsWork > 0 ? `${needsWork} 条发现需要补证或澄清边界；候选状态不会因报告材料未齐被判作低完成度。` : '当前活跃发现均已形成支撑性证据。'}</div></div></section>
}
